<div>
    ...
    @if (session()->has('message'))
        <div class="position-fixed top-0 start-50 translate-middle-x mt-3" style="z-index: 9999;">
            <div
                class="alert alert-success border-0 shadow-lg rounded-pill px-4 py-2 d-flex align-items-center animate__animated animate__fadeInDown">
                <i class="fa fa-check-circle me-2"></i>
                <span>{{ session('message') }}</span>
                <button type="button" class="btn-close ms-3 small" data-bs-dismiss="alert" aria-label="Close"
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <div class="input-group">
                    ...
                    <input type="text" class="form-control bg-light border-0" placeholder="Ketik nama..."
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative pengajuan-card">
                    ...
                    <div class="position-absolute" style="top: 15px; right: 15px; z-index: 10;">
                        <button class="btn btn-white btn-sm shadow-sm rounded-circle" data-bs-toggle="modal"
                            data-bs-target="#deleteConfirmModal" onclick="setDeleteId({{ $approval->id }})"
                            title="Hapus"
                    ...
                    <div class="card-body p-4 pt-5">
                        ...
                        <div class="bg-light rounded-4 p-3 mb-3 border-0">
                            <div class="d-flex justify-content-between mb-1">
                                ...
                                <span
                                    class="small fw-bold text-dark">{{ \Carbon\Carbon::parse($approval->tanggal_awal)->format('d M Y') }}</span>
                            </div>
                            @if ($approval->tanggal_akhir)
                                <div class="d-flex justify-content-between mb-1">
                                    <span class="text-muted small">Sampai:</span>
                                    <span
                                        class="small fw-bold text-dark">{{ \Carbon\Carbon::parse($approval->tanggal_akhir)->format('d M Y') }}</span>
                                </div>
                            @endif
                            <div class="mt-3">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <span class="small text-muted"><i class="fa fa-clock me-1"></i>
                                {{ $approval->created_at->diffForHumans() }}</span>
                            <span
                                class="badge bg-light text-{{ $approval->approval == 'Approved' ? 'success' : ($approval->approval == 'Rejected' ? 'danger' : 'warning') }} border py-1 px-3 rounded-pill">
                                {{ $approval->approval }}
                            </span>
                        </div>
                        ...
                        <div class="row g-2">
                            <div class="col-6">
                                <button class="btn btn-success btn-sm w-100 rounded-pill py-2 fw-bold"
                                    wire:click="approve({{ $approval->id }})">
                                    <i class="fa fa-check me-1"></i> Approve
                                </button>
                            </div>
                            <div class="col-6">
                                <button class="btn btn-outline-danger btn-sm w-100 rounded-pill py-2 fw-bold"
            ...
            <div class="modal-content border-0 shadow rounded-4">
                <div class="modal-body p-4 text-center">
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-center mt-4">
                        ...
                        <button type="button" class="btn btn-danger rounded-pill px-4" onclick="executeDelete()">Ya,
                            Hapus</button>
                    </div>
                </div>
            </div>
